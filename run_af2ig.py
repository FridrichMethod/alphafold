...
class DataPipeline:
    ...
    def process(
        ...
        sequence_features = make_sequence_features(sequence, domain_name)
        msa_features = make_msa_features(sequence, self.msa_features_for_all)
    ...
    def process_single_chain(
        ...
        if not is_homomer_or_monomer:
            pairing_msa_features = _convert_pairing_msa_features(
                make_msa_features(sequence, self.pairing_msa_features_for_all)
            )
...
def _make_all_chain_features_for_all(
    precalc_dir: str,
    msa_dir_name: str,
    template_dir_name: str,
    pairing_msa_dir_name: str,
) -> dict[str, DataPipelineSingleChain]:
    ...
    for chain_id in protein.PDB_CHAIN_IDS:
        ...
        dirs = {
            "msa_dir": os.path.join(precalc_dir, chain_id, msa_dir_name),
            "template_dir": os.path.join(precalc_dir, chain_id, template_dir_name),
            "pairing_msa_dir": os.path.join(
                precalc_dir, chain_id, pairing_msa_dir_name
            ),
        }

        # Only use paths that exist, otherwise use empty string
        dirs = {
            key: value if os.path.isdir(value) else "" for key, value in dirs.items()
        }
...
class DataPipelineMultimer:
    ...
    def __init__(
        ...
        self.all_chain_features_for_all = _make_all_chain_features_for_all(
...
class Af2Ig:
    ...
    def __init__(self, model_name: str, data_dir: str):
        ...
        self.is_multimer = "multimer" in model_name
...
def main(args):
    ...
    # load model
    af2ig = Af2Ig(model_name, data_dir)

    if af2ig.is_multimer:
        # Multimer arguments
    ...
    for record in SeqIO.parse(fasta_path, "fasta"):
        ...
        # process features
        try:
            feature_dict = data_pipeline.process(sequences, domain_name)
        except ValueError as e:
            logging.error(e)  # skip sequence
            continue

        # predict structure
        af2ig.predict(feature_dict, output_dir, random_seed=random_seed)
